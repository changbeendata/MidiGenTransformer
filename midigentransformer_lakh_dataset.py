# -*- coding: utf-8 -*-
"""MidiGenTransformer_lakh_Dataset.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16d8Tsh0CVLvjCsqG8Srh742r-IsqgDhl
...
my_drive_path = '/content/drive/MyDrive/been_colab/MIDI-Transformer/data/lakh/lakh' # your path to midi-files and select genre folder : Jazz, Pop, ...
...
for params in param_grid:
    print(f'd_model: {params[0]}, nhead: {params[1]}, num_encoder_layers: {params[2]}, dim_feedforward: {params[3]}, max_seq_len: {params[4]}')
...
model = TransformerModel(d_model, nhead, num_encoder_layers, dim_feedforward, max_seq_len).to('cuda:0')

# definition of loss function and optimizer
criterion = torch.nn.MSELoss() # performance measure is hyper..
optimizer = torch.optim.Adam(model.parameters(), lr=0.0005) # hyper..
...
# Animator
animator = d2l.Animator(xlabel='epoch', xlim=[1, num_epochs], ylim=[0, 3000],
...
for epoch in range(num_epochs):
    ...
    total_train_loss = 0
    for input_sequence, target_sequence in train_loader:
        input_sequence = input_sequence.to('cuda:0')
        target_sequence = target_sequence.to('cuda:0')

        optimizer.zero_grad()
        output = model(input_sequence).to('cuda:0')
        loss = criterion(output, target_sequence)
        loss.backward()
...
with torch.no_grad():
    for input_sequence, target_sequence in test_loader:
        output = model(input_sequence.to('cuda:0')).to('cuda:0')
        loss = criterion(output, target_sequence.to('cuda:0'))
        test_loss += loss.item()
...
output_folder = '/content/drive/MyDrive/been_colab/MIDI-Transformer/output_midi_lakh'
...
for i, (input_sequence, _) in enumerate(test_loader):
    ...
    model_output = model(input_sequence.to('cuda:0'))
